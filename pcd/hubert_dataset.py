...
class AVHubertDataset(FairseqDataset):
    def __init__(
        ...
        # count_of_each = len(self.names) // 3
        # type_list = [1] * count_of_each  + [2] * count_of_each + [3] * count_of_each 
        
        
        count_of_each = len(self.names) // 20
        x=9
        # type_list = [1] * count_of_each*x  + [2]  *count_of_each*x + [3] * count_of_each*(20-(2*x))
        # type_list = len(self.names)//2*[1] +len(self.names)//2*[2]
        type_list = len(self.names) * [2]
        remaining = len(self.names) - len(type_list)
        remaining_list = [1]*remaining
        type_list = type_list + remaining_list
        # 使用随机洗牌来打乱列表
        # np.random.shuffle(type_list)
        self.type_list=type_list
        
        # self.type_list=[1] * len(self.names)
        logger.info(
            f"finish type_list:{len(self.type_list)}"
        )
    ...
    def load_feature(self, mix_name,noise_prob1=0):
        """
        ...
        if 'audio' in self.modalities:
            ...
            # here
            # if np.random.rand() < noise_prob1:
            #     wav_data = self.add_noise(wav_data)
            # logger.info(f"wav_data{wav_data.shape}")
            if np.random.rand() < noise_prob1:
                if os.path.exists(audio_fn.replace('.wav', '_noise_snr-10.npy')):
                    audio_feats = np.load(audio_fn.replace('.wav', '_noise_snr-10.npy'))
                else:
                    wav_data = self.add_noise(wav_data)
                    audio_feats = logfbank(wav_data, samplerate=sample_rate).astype(np.float32)  # [T, F]
                    audio_feats = stacker(audio_feats, self.stack_order_audio)  # [T/stack_order_audio, F*stack_order_audio]
                    np.save(audio_fn.replace('.wav', '_noise_snr-10.npy'), audio_feats)
                # noise_id = np.random.randint(0, 8)
                # audio_feats = np.load(audio_fn.replace('.wav', f'_noise{noise_id}.npy'))
                # wav_data = self.add_noise(wav_data)
            else:
                if os.path.exists(audio_fn.replace('.wav', '.npy')):
                    audio_feats = np.load(audio_fn.replace('.wav', '.npy'))
                else:
                    audio_feats = logfbank(wav_data, samplerate=sample_rate).astype(np.float32)  # [T, F]
                    audio_feats = stacker(audio_feats, self.stack_order_audio)  # [T/stack_order_audio, F*stack_order_audio]
                    np.save(audio_fn.replace('.wav', '.npy'), audio_feats)
                # logger.info(f"audio_fn{audio_fn.shape}")
            # audio_feats = logfbank(wav_data, samplerate=sample_rate).astype(np.float32)  # [T, F]
            # audio_feats = stacker(audio_feats, self.stack_order_audio)  # [T/stack_order_audio, F*stack_order_audio]
        else:
            audio_feats = None
        ...
        # logger.info(f"audio_feats{audio_feats.shape}")
    ...
    def __getitem__(self, index):
        # logger.info(f"fnmiaosfbhnpalsiefnbpiowa={index}")
        video_feats, audio_feats = self.load_feature(self.names[index])
        
        
        # vmissing_prob = self.noise_prob
        # if self.type_list[index]==1:
        #     if video_feats is not None:
        #         for i in range(video_feats.shape[0]):
        #             if np.random.random() < vmissing_prob:
        #                 video_feats[i] = np.random.uniform(np.min(video_feats[i]),np.max(video_feats[i]),video_feats[i].shape) # mask0
                    
        # elif self.type_list[index]==2:
        #     if audio_feats is not None:
        #         for i in range(audio_feats.shape[0]):
        #             if np.random.random() < self.noise_prob:
        #                 audio_feats[i] = np.random.uniform(np.min(audio_feats[i]),np.max(audio_feats[i]),audio_feats[i].shape) # maska0
                        
        # for i in range(video_feats.shape[0]):
        #     video_feats[i] = np.random.uniform(0,0,video_feats[i].shape) # mask0

        vnoise_prob = self.noise_prob
        # video加了噪音
        if self.type_list[index]==1:
            if video_feats is not None:
                # noise2_continuous
                ...
                if vnoise_prob==1:
                    ...
                    start_index = np.random.randint(0, video_feats.shape[0]*(1-vnoise_prob))  # 从0到 video_feats.shape[0] - 1 之间随机选择一个起始索引
                segment_length = int(video_feats.shape[0]*vnoise_prob)

                # 2. 计算结束索引
                end_index = start_index + segment_length
                if end_index > video_feats.shape[0]:
                    end_index = video_feats.shape[0]
                while start_index<end_index:
                    another_name = np.random.choice([name[0] for name in self.names])
                    another_video = self.load_video(another_name)
                    if another_video.shape[0]>end_index-start_index:
                        if another_video.shape[0]-(end_index-start_index)==0:
                            another_start=0
                        else:
                            another_start=np.random.randint(0, another_video.shape[0]-(end_index-start_index))
                        video_feats[start_index:end_index] = another_video[another_start:another_start + (end_index - start_index)]
                        break
                    else:  
                        video_feats[start_index:start_index + another_video.shape[0]] = another_video[:]
                        start_index = start_index + another_video.shape[0]
        # audio加了噪音
        elif self.type_list[index]==2:
            # 1. 选择一个起始索引和连续片段的长度
                if vnoise_prob==1:
                    ...
                    start_index = np.random.randint(0, audio_feats.shape[0]*(1-vnoise_prob))  # 从0到 video_feats.shape[0] - 1 之间随机选择一个起始索引
                segment_length = int(audio_feats.shape[0]*vnoise_prob)

                # 2. 计算结束索引
                end_index = start_index + segment_length
                if end_index > audio_feats.shape[0]:
                    end_index = audio_feats.shape[0]
                while start_index<end_index:
                    another_name = np.random.randint(0, len(self.names))
                    _, another_audio = self.load_feature(self.names[another_name])
                    if another_audio.shape[0]>end_index-start_index:
                        if another_audio.shape[0]-(end_index-start_index)==0:
                            another_start=0
                        else:
                            another_start=np.random.randint(0, another_audio.shape[0]-(end_index-start_index))
                        audio_feats[start_index:end_index] = another_audio[another_start:another_start + (end_index - start_index)]
                        break
                    else:  
                        audio_feats[start_index:start_index + another_audio.shape[0]] = another_audio[:]
                        start_index = start_index + another_audio.shape[0]
        ...
        #             another_name = np.random.choice([name[0] for name in self.names])
        #             another_video = self.load_video(another_name)
        #             for i in range(another_video.shape[0]):
        #                 another_video[i] = np.random.uniform(np.min(another_video[i]),np.max(another_video[i]),another_video[i].shape) # mask0
        ...
        #         while start_index<end_index:
        #             another_name = np.random.randint(0, len(self.names))
        #             _, another_audio = self.load_feature(self.names[index],1)
        #             audio_feats[start_index:end_index] = another_audio[start_index:end_index]
        ...
        #         a,b = self.names[index]
        #         another_video = self.load_video(a)
        #         another_start=video_feats.shape[0]-segment_length
        #         video_feats[start_index:end_index] = another_video[another_start:another_start + segment_length]
        #         # audio加了噪音
        # elif self.type_list[index]==2:
        #     # 1. 选择一个起始索引和连续片段的长度
        #     start_index=0
        ...
        #     _, another_audio = self.load_feature(self.names[index])
        #     another_start=audio_feats.shape[0]-segment_length
        #     audio_feats[start_index:end_index] = another_audio[another_start:another_start + (end_index - start_index)]
        ...
        # 这里采集原始数据
        # video_feats, audio_feats = self.load_feature(self.names[index]) 
        # video_feats2, audio_feats2 = self.load_feature(self.names[index])
        # audio_feats2, video_feats2 = torch.from_numpy(
        #     audio_feats2.astype(np.float32)) if audio_feats2 is not None else None, torch.from_numpy(
        #     video_feats2.astype(np.float32)) if video_feats2 is not None else None
        # if self.normalize and 'audio' in self.modalities:
        #     with torch.no_grad():
        #         audio_feats2 = F.layer_norm(audio_feats2, audio_feats2.shape[1:])
        ...
        if self.normalize and audio_feats is not None:
            with torch.no_grad():
        ...
        # logger.info(f"audio_size1{audio_feats.shape}")
        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels, "type":self.type_list[index]}
    ...
    def collater(self, samples):
        ...
        # logger.info(f"audio_size2{collated_audios.shape}")

        # 采集完整数据
        # audio_source2, video_source2 = [s["audio_source2"] for s in samples], [s["video_source2"] for s in samples]
        # if audio_source2[0] is None:
        #     audio_source2 = None
        # if video_source2[0] is None:
        #     video_source2 = None
        # if audio_source2 is not None:
        #     collated_audios2, padding_mask, audio_starts = self.collater_audio(audio_source2, audio_size)
        # else:
        #     collated_audios2, audio_starts = None, None
        # if video_source2 is not None:
        #     collated_videos2, padding_mask, audio_starts = self.collater_audio(video_source2, audio_size, audio_starts)
        # else:
        #     collated_videos2 = None
        
        
        source = {"audio": collated_audios, "video": collated_videos }
        net_input = {"source": source, "padding_mask": padding_mask,"type": [s['type'] for s in samples]}
        
        # source2= {"audio2":collated_audios2,"video2":collated_videos2}
        # net_input2 = {"source": source2, "padding_mask": padding_mask,"type": [s['type'] for s in samples]}
