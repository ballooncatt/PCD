...
class Prompt_Base(nn.Module):
    ...
    def forward(self, x):
        # logger.info(f"fnhsoanfoasfpasmfcp1:{x.size()}")
        soft_weight = self.meta(x)  # B,T,C
        # logger.info(f"fnhsoanfoasfpasmfcp2:{soft_weight.size()}")
        soft_weight = torch.softmax(soft_weight, dim=-1)
        # logger.info(f"fnhsoanfoasfpasmfcp3:{soft_weight.size()}")
        return x + soft_weight @ self.emb_center

# 没聚类，直接拼接
class Prompt_Base0(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        super().__init__()
        self.encoder_embed_dim = encoder_embed_dim
        self.dropout = dropout
        ...
        prompt_length = 2
        self.emb_center_1 = nn.Parameter(torch.empty(prompt_length , self.encoder_embed_dim))
        self.emb_center_2 = nn.Parameter(torch.empty(prompt_length , self.encoder_embed_dim))
        self.emb_center_3 = nn.Parameter(torch.empty(prompt_length , self.encoder_embed_dim))
        
        nn.init.normal_(self.emb_center_1, std=64 ** -0.5)
        nn.init.normal_(self.emb_center_2, std=64 ** -0.5)
        nn.init.normal_(self.emb_center_3, std=64 ** -0.5)
    ...
    def forward(self, x, type):
        ...
        for i in range(0,B):
            if type[i]==1:
                prompt =self.emb_center_1#T,D
            elif type[i]==2:
                prompt = self.emb_center_2
            else:
                prompt =  self.emb_center_3
            ...
            if prompts is None:
                ...
                prompts = torch.cat([prompts, prompt], dim=0)#B,T,D
                
        return torch.cat([prompts,x],dim=1).transpose(0,1)  

# 一个聚类器
class Prompt_Base1(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        super().__init__()
        self.encoder_embed_dim = encoder_embed_dim
        self.dropout = dropout
        ...
        self.emb_center_1 = nn.Parameter(torch.empty(self.center_K, self.encoder_embed_dim))
        self.emb_center_2 = nn.Parameter(torch.empty(self.center_K, self.encoder_embed_dim))
        self.emb_center_3 = nn.Parameter(torch.empty(self.center_K, self.encoder_embed_dim))
    ...
    def forward(self, x, type):
        ...
        prompts=None
        for i in range(0,B):
            soft_weight = self.meta(x[i,:,:])
            soft_weight = torch.softmax(soft_weight, dim=-1)#T,C
            if type[i]==1:
                prompt = soft_weight @ self.emb_center_1#T,D
            elif type[i]==2:
                prompt = soft_weight @ self.emb_center_2
            else:
                prompt = soft_weight @ self.emb_center_3
        ...
        return (x + prompts).transpose(0,1)#T,B,D

# 三个聚类器
class Prompt_Base2(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        super().__init__()
        self.encoder_embed_dim = encoder_embed_dim
        self.dropout = dropout
        ...
        self.meta1 = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(self.encoder_embed_dim, self.center_K))
        ]))
        self.meta2 = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(self.encoder_embed_dim, self.center_K))
        ]))
        self.meta3 = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(self.encoder_embed_dim, self.center_K))
        ]))
    ...
    def forward(self, x, type):
        ...
        for i in range(0,B):  
            if type[i]==1:
                soft_weight = self.meta1(x[i,:,:])
                soft_weight = torch.softmax(soft_weight, dim=-1)#T,C
                prompt = soft_weight @ self.emb_center_1#T,D
            elif type[i]==2:
                soft_weight = self.meta2(x[i,:,:])
                soft_weight = torch.softmax(soft_weight, dim=-1)#T,C
                prompt = soft_weight @ self.emb_center_2
            else:
                soft_weight = self.meta3(x[i,:,:])
                soft_weight = torch.softmax(soft_weight, dim=-1)#T,C
                prompt = soft_weight @ self.emb_center_3
...
class Prompt_input(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        ...
        prompt_length = 16
        # self.emb_center = nn.Parameter(torch.empty(self.center_K, self.encoder_embed_dim))
        self.vn_prompt = nn.Parameter(torch.empty(self.center_K, prompt_length ,self.encoder_embed_dim))
        self.an_prompt = nn.Parameter(torch.empty(self.center_K, prompt_length ,self.encoder_embed_dim))
        self.all_prompt = nn.Parameter(torch.empty(self.center_K, prompt_length ,self.encoder_embed_dim))

        # nn.init.normal_(self.emb_center, std=64 ** -0.5)
        nn.init.normal_(self.vn_prompt, std=64 ** -0.5)
        nn.init.normal_(self.an_prompt, std=64 ** -0.5)
        nn.init.normal_(self.all_prompt, std=64 ** -0.5)
    ...
    def forward(self, x, type):
        ...
        soft_weight = self.meta(x)  # B,T,C
        # logger.info(f"fnhsoanfoasfpasmfcp2:{soft_weight.size()}")
        soft_weight = torch.softmax(soft_weight, dim=-1) #B,T,C
        # logger.info(f"fnhsoanfoasfpasmfcp3:{soft_weight.size()}")
        # 这里给每一个x聚个类
        x_weight = soft_weight.sum(dim=1)
        B,C =x_weight.size()
        # logger.info(f"fnhsoanfoasfpasmfcp4:{B,len(type)}")
        prompts = None
        for i in range(0,B):
            xx = x_weight[i,:]
            if type[i]==1:
                prompt = self.vn_prompt.transpose(0,2) @ xx 
            elif type[i]==2:
                prompt = self.an_prompt.transpose(0,2) @ xx 
            else:
                prompt = self.all_prompt.transpose(0,2) @ xx 
                
            prompt = prompt.transpose(0,1).unsqueeze(0)
            # logger.info(f"fnhsoanfoasfpasmfcp4:{prompt.size()}")
            if prompts is None:
                prompts = prompt
            else:
                prompts = torch.cat([prompts, prompt], dim=0)
        return torch.cat([prompts,x],dim=1).transpose(0,1)  
    # centerk=100, encoder_embed_dim=768, x:B,T,D,
    # [2024-01-06 17:51:48,850][opensr.hubert][INFO] - fnhsoanfoasfpasmfcp1:torch.Size([78, 8, 768])
    # [2024-01-06 17:51:48,851][opensr.hubert][INFO] - fnhsoanfoasfpasmfcp2:torch.Size([78, 8, 100])
    # [2024-01-06 17:51:48,851][opensr.hubert][INFO] - fnhsoanfoasfpasmfcp3:torch.Size([78, 8, 100])

class Prompt_input3(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        super().__init__()
        self.encoder_embed_dim = encoder_embed_dim
        self.dropout = dropout
    ...
    def forward(self, x, type):
        ...
        for i in range(0,B):
            ...
            if type[i]==1:
                soft_weight = self.meta1(x)  # B,T,C
                # logger.info(f"fnhsoanfoasfpasmfcp2:{soft_weight.size()}")
                soft_weight = torch.softmax(soft_weight, dim=-1) #B,T,C
                # logger.info(f"fnhsoanfoasfpasmfcp3:{soft_weight.size()}")
                # 这里给每一个x聚个类
                ...
                soft_weight = self.meta2(x)  # B,T,C
                # logger.info(f"fnhsoanfoasfpasmfcp2:{soft_weight.size()}")
                soft_weight = torch.softmax(soft_weight, dim=-1) #B,T,C
                # logger.info(f"fnhsoanfoasfpasmfcp3:{soft_weight.size()}")
                # 这里给每一个x聚个类
                ...
                soft_weight = self.meta3(x)  # B,T,C
                # logger.info(f"fnhsoanfoasfpasmfcp2:{soft_weight.size()}")
                soft_weight = torch.softmax(soft_weight, dim=-1) #B,T,C
                # logger.info(f"fnhsoanfoasfpasmfcp3:{soft_weight.size()}")
                # 这里给每一个x聚个类
...
class Prompt_inputx(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        ...
        low_rank=4
        prompt_length = 8
        self.common_prompt = nn.Parameter(torch.empty((prompt_length, low_rank, self.encoder_embed_dim )))
        self.specific_prompt = nn.Parameter(torch.empty((prompt_length, 3, low_rank))  ) 
        self.common_prompt.normal(mean=0.0,std=0.02)
        self.specific_prompt.normal(mean=0.0,std=0.02)     
        # self.emb_center = nn.Parameter(torch.empty(self.center_K, self.encoder_embed_dim))
        self.vn_prompt = self.specific_prompt @ self.common_prompt[:,0,:]
        self.an_prompt = self.specific_prompt @ self.common_prompt[:,1,:]
        self.all_prompt = self.specific_prompt @ self.common_prompt[:,2,:]

        # self.vn_prompt[1:2,:].fill_(1)
        # self.an_prompt[2:3,:].fill_(1)
        # self.all_prompt[0:1,:].fill_(1)
        # nn.init.normal_(self.emb_center, std=64 ** -0.5)
        # nn.init.normal_(self.vn_prompt, std=64 ** -0.5)
        # nn.init.normal_(self.an_prompt, std=64 ** -0.5)
        # nn.init.normal_(self.all_prompt, std=64 ** -0.5)
    ...
    def forward(self, x, type):
        ...
        for i in range(0,B):
            if type[i]==1:
                prompt = self.vn_prompt
            elif type[i]==2:
                prompt = self.an_prompt
            else:
                prompt = self.all_prompt
...
class Prompt_input0(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        ...
        prompt_length = 8
        # self.emb_center = nn.Parameter(torch.empty(self.center_K, self.encoder_embed_dim))
        self.vn_prompt = nn.Parameter(torch.empty( prompt_length ,self.encoder_embed_dim))
        self.an_prompt = nn.Parameter(torch.empty( prompt_length ,self.encoder_embed_dim))
        self.all_prompt = nn.Parameter(torch.empty( prompt_length ,self.encoder_embed_dim))

        # self.vn_prompt[1:2,:].fill_(1)
        # self.an_prompt[2:3,:].fill_(1)
        # self.all_prompt[0:1,:].fill_(1)
        nn.init.constant_(self.vn_prompt[1:2, :], 1)
        nn.init.constant_(self.an_prompt[2:3, :], 1)
        nn.init.constant_(self.all_prompt[0:1, :], 1)
        # nn.init.normal_(self.emb_center, std=64 ** -0.5)
        # nn.init.normal_(self.vn_prompt, std=64 ** -0.5)
        # nn.init.normal_(self.an_prompt, std=64 ** -0.5)
        # nn.init.normal_(self.all_prompt, std=64 ** -0.5)
...
class Prompt_msa0(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        ...
        prompt_length = 32
        # self.emb_center = nn.Parameter(torch.empty(self.center_K, self.encoder_embed_dim))
        self.vn_prompt_k = nn.Parameter(torch.empty( prompt_length//2 ,self.encoder_embed_dim))
        self.an_prompt_k = nn.Parameter(torch.empty( prompt_length//2 ,self.encoder_embed_dim))
        self.all_prompt_k = nn.Parameter(torch.empty( prompt_length//2 ,self.encoder_embed_dim))

        self.vn_prompt_v = nn.Parameter(torch.empty( prompt_length//2 ,self.encoder_embed_dim))
        self.an_prompt_v = nn.Parameter(torch.empty( prompt_length//2 ,self.encoder_embed_dim))
        self.all_prompt_v = nn.Parameter(torch.empty( prompt_length//2 ,self.encoder_embed_dim))

        # self.vn_prompt[1:2,:].fill_(1)
        # self.an_prompt[2:3,:].fill_(1)
        # self.all_prompt[0:1,:].fill_(1)
        nn.init.constant_(self.vn_prompt_k[1:2, :], 1)
        nn.init.constant_(self.an_prompt_k[2:3, :], 1)
        nn.init.constant_(self.all_prompt_k[0:1, :], 1)
        nn.init.constant_(self.vn_prompt_v[1:2, :], 1)
        nn.init.constant_(self.an_prompt_v[2:3, :], 1)
        nn.init.constant_(self.all_prompt_v[0:1, :], 1)
        # nn.init.normal_(self.emb_center, std=64 ** -0.5)
        # nn.init.normal_(self.vn_prompt, std=64 ** -0.5)
        # nn.init.normal_(self.an_prompt, std=64 ** -0.5)
        # nn.init.normal_(self.all_prompt, std=64 ** -0.5)
    ...
    def forward(self, x, type):
        ...
        for i in range(0,B):
            if type[i]==1:
                prompt_k = self.vn_prompt_k
                prompt_v = self.vn_prompt_v

            elif type[i]==2:
                prompt_k = self.an_prompt_k
                prompt_v = self.an_prompt_v
            else:
                prompt_k = self.all_prompt_k
                prompt_v = self.all_prompt_v
            prompt_k = prompt_k.unsqueeze(0)
            prompt_v = prompt_v.unsqueeze(0)

            # logger.info(f"fnhsoanfoasfpasmfcp4:{prompt.size()}")
            if prompts_k is None:
                prompts_k = prompt_k
            else:
                prompts_k = torch.cat([prompts_k, prompt_k], dim=0)
            if prompts_v is None:
                prompts_v = prompt_v
            else:
                prompts_v = torch.cat([prompts_v, prompt_v], dim=0)
        return x,prompts_k.transpose(0,1), prompts_v.transpose(0,1)  

class Prompt_msa3(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        super().__init__()
        self.encoder_embed_dim = encoder_embed_dim
        self.dropout = dropout
    ...
    def forward(self, x, type):
        ...
        for i in range(0,B):
            ...
            if prompts is None:
                ...
                prompts = torch.cat([prompts, prompt], dim=0)
        x=x.transpose(0,1)
        return prompts[:,self.prompt_length//2:,:].transpose(0,1),prompts[:,:self.prompt_length//2,:].transpose(0,1) 


class Prompt_msa(nn.Module):
    def __init__(self, encoder_embed_dim, center_K=100, dropout=0.1):
        super().__init__()
        self.encoder_embed_dim = encoder_embed_dim
        self.dropout = dropout
...
class PromptLearner(nn.Module):
    def __init__(self, encoder_embed_dim, strategy, training):
        ...
        if self.strategy == 'modal_prompt':
            ...
            prompt_embed_dict = {}
            for layer in range(0, 24):
                prompt_embed_dict.setdefault(str(layer), Prompt_Base2(self.encoder_embed_dim, center_K=center_K))

            self.prompt_embed = nn.ModuleDict(prompt_embed_dict)
            self.prompt_embeddings = ('base', self.prompt_embed)
        elif 'plus' in self.strategy:
            center_K = int(self.strategy.split('_')[-1])
            prompt_embed_dict = {}
            for layer in range(0, 24):
                prompt_embed_dict.setdefault(str(layer), Prompt_Base2(self.encoder_embed_dim, center_K=center_K))
                # prompt_embed_dict.setdefault(str(layer), Prompt_Base1(self.encoder_embed_dim, center_K=center_K))

            self.prompt_embed = nn.ModuleDict(prompt_embed_dict)
            self.prompt_embeddings = ('plus', self.prompt_embed)
            
        elif 'input' in self.strategy:
            center_K = int(self.strategy.split('_')[-1])
            prompt_embed_dict = {}
            for layer in range(0, 24):
                # prompt_embed_dict.setdefault(str(layer), Prompt_Base1(self.encoder_embed_dim, center_K=center_K))
                prompt_embed_dict.setdefault(str(layer), Prompt_input0(self.encoder_embed_dim, center_K=center_K))

            self.prompt_embed = nn.ModuleDict(prompt_embed_dict)
            self.prompt_embeddings = ('input', self.prompt_embed)
        elif 'msa' in self.strategy:
            center_K = int(self.strategy.split('_')[-1])  
            prompt_embed_dict = {}
            for layer in range(0, 24):
                # prompt_embed_dict.setdefault(str(layer), Prompt_Base1(self.encoder_embed_dim, center_K=center_K))
                prompt_embed_dict.setdefault(str(layer), Prompt_msa0(self.encoder_embed_dim, center_K=center_K))

            self.prompt_embed = nn.ModuleDict(prompt_embed_dict)
            self.prompt_embeddings = ('msa', self.prompt_embed)
        else:
...
class AVHubertModel(BaseFairseqModel):
    def __init__(
        ...
        self.encoder = TransformerEncoder_prompt(cfg)
    ...
    def extract_finetune(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None,type=None):
        src_audio, src_video = source['audio'], source['video']
        # logger.info(f"{src_audio.shape}")
        if mask and self.masking_type == 'input':
            src_video, mask_indices_video = self.apply_input_mask(src_video, padding_mask, target_list=None)
            src_audio, mask_indices_audio = self.apply_input_mask(src_audio, padding_mask, target_list=None)
            mask_indices = torch.logical_or(mask_indices_audio,
    ...
    def extract_prompt(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None,type=None):
        src_audio, src_video = source['audio'], source['video']
        ...
        #     if type[i]==1:
        #         if np.random.random() < vmissing_prob:
        #             features_video[i].zero_()
        #     if type[i]==2:
        #         if np.random.random() < vmissing_prob:
        #             features_audio[i].zero_()        
        ...
        # logger.info(f"nsioanoasn1:features{features.size()},embdeing:{self.encoder_embed_dim}")
        ...
        # padding_mask: (B, T), bool
        # mask_indices: (B, T), bool
        # if torch.any(padding_mask):
        #     logger.info(f"its true{padding_mask}")
        x, _, padding_mask = self.encoder(
            x,
            padding_mask=padding_mask,
            layer=None if output_layer is None else output_layer - 1,
